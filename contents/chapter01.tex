...
This thesis investigates two complementary questions: (i)~whether a \emph{token-level} approach, preserving the full sequence of token embeddings and employing parameter-efficient fine-tuning, can substantially improve metagenomic taxonomic classification over frozen-embedding baselines; and (ii)~whether GFM pre-training can compensate for a short non-overlapping $k$-mer tokenizer, or whether tokenization imposes a structural ceiling that neither depth nor pre-training can raise. A third, operational question follows from (ii): if pre-training does not raise the 50M-read ceiling, does it still buy data efficiency when labelled reads are scarce?
...
\begin{enumerate}
    ...
    \item \textbf{Pre-training vs.\ depth vs.\ tokenization}: Separate the contributions of large-scale genomic pre-training, encoder depth, and $k$-mer tokenization, including a depth-matched from-scratch control that the one-layer versus 29-layer comparison does not provide, and identify the dominant bottleneck of the proposed approach.
...
\begin{enumerate}
    ...
    \item \textbf{Pre-training buys data efficiency, not a higher 50M ceiling}: A one-layer randomly initialized Transformer under the same 6-mer tokenizer, 50M reads, and head trails 29-layer NT-v2 by $+13.19$~pp RC TTA (67.07\% vs.\ 53.88\%). That comparison confounds pre-training with depth and width. A depth sweep with everything else held fixed (NT-v2 6-mer tokenizer, 50M gut reads, attention-pooling head, RC TTA, the same \texttt{clean\_common} 100K file) scores 53.92\% / 65.44\% / 67.94\% at 1 / 8 / 16 from-scratch layers, against 67.08\% for NT-v2 LoRA. Depth accounts for $+14.02$~pp ($1\to 16$); at matched 6-mer tokenization and comparable depth the net contribution of pre-training at 50M is $\approx 0$ (the 16-layer from-scratch model is $+0.86$~pp higher, with $129\times$ fewer parameters). The same 16-layer comparison at 5M reads reverses the ranking: NT-v2 63.05\% versus from-scratch 54.11\% ($+8.94$~pp). Pre-training therefore helps when labels are scarce; it does not raise the 6-mer ceiling once 50M labelled reads are available. The 5M NT-v2 figure is the recorded internal-validation number for that checkpoint; every model we could re-score agrees across the two pools (Section~\ref{sec:exp-shallow}).

    \item \textbf{Tokenization is the dominant factor, including at matched architecture}: Holding MetaTransformer's architecture fixed and varying $k \in \{6, 12, 13\}$ and stride exposes two compounding effects: overlapping stride ($s{=}1$) vs.\ non-overlapping ($s{=}k$) accounts for up to $+60$~pp at $k{=}13$, and longer $k$ monotonically improves accuracy among overlapping configurations. A MetaTransformer with overlapping 13-mer tokenization trained from scratch surpasses NT-v2 + LoRA by $+20.35$~pp (87.42\% vs.\ 67.07\%). A same-architecture 16-layer comparison on \texttt{clean\_common} RC TTA reaches 83.83\% with a hashed 13-mer (2~GiB) and 85.63\% with an exact 13-mer, versus 67.94\% for the 16-layer non-overlapping 6-mer; a 29-layer from-scratch 6-mer reaches only 69.52\%. Depth can compensate for a weak tokenizer; it does not break the 6-mer ceiling. The 6-mer constraint---not backbone capacity, LoRA, or pre-training---is the principal bottleneck.

    \item \textbf{Reverse-complement test-time augmentation}: We evaluate RC TTA---averaging logits from forward and reverse-complement strand passes at inference---across all experimental settings and architectures. RC TTA delivers a consistent, zero-training-cost improvement of $+0.1$ to $+1.5$~pp, with a clear differential between pre-trained models ($+0.78$--$+1.54$~pp) and randomly initialized models ($\approx$$+0.08$~pp), implicating pre-training-induced strand bias as the primary mechanism.
