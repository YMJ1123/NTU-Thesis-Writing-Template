...
\begin{enumerate}
    ...
    \item \textbf{Data scaling saturates beyond 50M}: We trained balanced subsets up to 250M reads (Section~\ref{sec:exp-scaling}); the warm-started 250M model reaches only 67.29\% RC TTA ($+0.22$~pp over the 50M 67.07\%), confirming that accuracy saturates rather than following the log-linear projection. Because the reference provides a single genome per species, reads beyond $\sim$50M redundantly re-cover the same sequences; the gap to overlapping-$k$-mer methods is therefore not closable by additional training data within the NT-v2 6-mer framework.
