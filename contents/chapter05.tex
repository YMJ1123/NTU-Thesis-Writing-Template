...
\subsection{Full-Scale and Streaming Training}
...
Per-genus 50M classifiers under oracle-genus routing reach 27.8\% species Top-1 (Section~\ref{sec:spv4-hierarchical-prelim}), exceeding the flat sp\_v4 oracle (27.4\%) and validating per-genus specialisation. Predicted routing through the 66\%--67\% genus model, however, lowers Top-1 to 14.7\%---below the flat baseline---because per-genus specialists do not recognise reads from genera they were not trained on. The most direct path to closing this gap is a three-layer routing architecture: partitioning the five largest genera (\textit{Collinsella}, \textit{Clostridium}, \textit{Prevotella}, \textit{Bacteroides}, \textit{Blautia}) into subgroups of 25--30 species, training a per-subgenus router, and only descending to a per-genus specialist after subgenus disambiguation. Improving genus-routing accuracy itself is bounded by NT-v2's 6-mer tokenization and is therefore best addressed indirectly through the long-$k$ pre-training direction above.
