...
ART is invoked without a fixed \texttt{--rndSeed}, so the underlying $\sim\!258$M-read \texttt{reads.fa} is not bit-for-bit reproducible (a fresh ART run would draw new noise realisations and read positions), although its statistical properties---per-genome coverage, label distribution, and read-length distribution---are reproducible by construction.
Downstream of the simulator, the balanced-subsampling stage uses a fixed seed (\texttt{seed=42}), so all balanced subsamples (500K, 5M, 50M) used in this thesis are exactly reproducible from a given \texttt{reads.fa}.
This asymmetry is acknowledged as a methodological limitation and is the dominant non-determinism in the data pipeline.
