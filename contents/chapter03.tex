...
\subsection{Two-Phase Training and Transfer Between Experiments}

Training proceeds in two phases: the backbone is frozen while the classification head
warms up, then LoRA adapters on the query/key/value projections are unfrozen and both
are trained jointly. Each larger-scale run is initialised from the best checkpoint of
the scale below it (\texttt{--init\_from}), which loads matching LoRA parameters and
skips the classification head whenever the label space changes---as it does between the
120-class genus and 1{,}535-class species models. Warm-starting this way is what makes
the data-scaling series a series rather than a set of independent runs, and it is also
why the 250M genus arm is reported both warm-started and from scratch
