...
\begin{figure}[htbp]
    ...
    \caption{Overview of the proposed token-level GFM classification pipeline. A 150~bp DNA read is tokenized into 6-mers, encoded by the LoRA-adapted Nucleotide Transformer v2 backbone, aggregated via attention pooling, and classified into one of $K$ taxonomic classes.}
...
We employ the \texttt{nucleotide-transformer-v2-500m-multi-species} model as the backbone encoder. This model is based on the ESM-2 architecture~\cite{Lin2023}, a protein language model architecture adapted for nucleotide sequences.
