...
\textbf{Q2} Tokenization wins ($+20.35$~pp); old +13.19 ``pre-training'' gap was mostly depth
\end{block}
\end{columns}
\end{frame}
...
\node[label, below=1mm of v11] {\textbf{1L control (depth confound)}};
...
% -----------------------------------------------------------------------
\begin{frame}{Finding 2 — Depth Explains the Old Pre-training Gap}
\begin{columns}
\column{0.55\textwidth}
\includegraphics[width=\linewidth]{../figures/depth_sweep_6mer.pdf}

\column{0.45\textwidth}
\begin{tabular}{lc}
\toprule
Model (6-mer, 50M) & Acc (RC TTA) \\ \midrule
1-layer scratch & 53.92\% \\
16-layer scratch & \textbf{67.94\%} \\
29-layer scratch & 69.52\% \\
NT-v2 LoRA (29L) & 67.08\% \\
\midrule
\textbf{Old 29L vs 1L} & +13.19 (\emph{depth}) \\
\bottomrule
\end{tabular}

\vspace{8pt}
{\small 舊 +13.19 混到深度；matched depth 時 pre-training ≈ 0（5M 才有 data efficiency）}
\end{columns}
\end{frame}
...
\node[step, right=of s1] (s2) {Depth explains +13.19;\\50M matched depth\\pre-train $\approx 0$};
...
\begin{enumerate}
  ...
  \item Depth explains old $+13.19$~pp; pre-training = low-data efficiency
